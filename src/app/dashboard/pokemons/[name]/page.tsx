...
export async function generateStaticParams() {
  const pokemons: PokemonsResponse = await fetch('https://pokeapi.co/api/v2/pokemon?limit=151')
    .then(resp => resp.json());
  const static151Pokemons = pokemons.results.map(({name}) => name);
...
export async function generateMetadata({params}:Props): Promise<Metadata> {
  try {
    ...
    return {
      ...
      description: 'Algo salió mal en la metadata'
    };
  }
}
...
const getPokemon = async(name: string):Promise<Pokemon> => {
  try {
    const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, {
      cache: 'force-cache',
      // Revalidate the page automaticly 
      // next: {
